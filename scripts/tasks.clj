...
(defn- sha256-sri
  ...
  (let [digest (.digest (java.security.MessageDigest/getInstance "SHA-256")
        ...
        encoded (.encodeToString (java.util.Base64/getEncoder) digest)]
    (str "sha256-" encoded)))

(defn- remote-refs-hash
  [url]
  (let [{:keys [out err exit]} (p/shell {:out :string :err :string :continue true}
        ...
        canonical (str/join "\n" lines)]
    (when-not (zero? exit)
      (throw (ex-info "Command failed: git ls-remote <url> --heads --tags"
                      {:url url :exit exit :err err})))
    (sha256-sri canonical)))
...
(defn- nix-source-hash-expr
  [docs-root url rev]
  (str "let\n"
       "  flake = builtins.getFlake " (nix-string docs-root) ";\n"
...
(defn- prefetch-hash
  [docs-root url rev]
  (let [expr (nix-source-hash-expr docs-root url rev)
             ...
             (map (fn [[_ key body]]
                    [key
                     {:url (some-> (body-field body "url") normalize-git-url)
                      ...
                      :refs-hash (body-field body "refs_hash")
...
(defn- render-project
  [[project-name {:keys [url rev hash refs-hash branches start-path]}]]
  (str "  " (nix-string project-name) " = {\n"
       "    url = " (nix-string url) ";\n"
       "    rev = " (nix-string rev) ";\n"
       "    hash = " (nix-string hash) ";\n"
       "    refs_hash = " (nix-string refs-hash) ";\n"
...
(defn update-projects
  ...
  Performance: when both a project's remote HEAD `rev` and remote refs hash are
  unchanged, we reuse the existing source hash from `pkgs/projects.nix`."
  []
  (let [docs-root (str (fs/absolutize "."))
        existing-projects (parse-projects docs-root)
        projects (->> existing-projects
                      (map (fn [[project-name {:keys [url rev hash refs-hash branches start-path]}]]
                             (when (str/blank? url)
                             ...
                             (let [remote-url (normalize-git-url url)
                                   ...
                                   latest-refs-hash (remote-refs-hash remote-url)
                                   latest-hash (if (and (= latest-rev rev)
                                                        (= latest-refs-hash refs-hash)
                                                        (not (str/blank? hash)))
                                                 hash
                                                 (prefetch-hash docs-root remote-url latest-rev))]
                               [project-name
                                {:url remote-url
                                 :rev latest-rev
                                 :hash latest-hash
                                 :refs-hash latest-refs-hash
                                 :branches branches

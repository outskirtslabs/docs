...
function versionTree(components, page) {
  ...
  for (const [, component] of Object.entries(components)) {
    const componentVersionToUrl =
      component && page && page.component && component.name === page.component.name ? versionToUrl : {}
...
function splitVersions(versions, versionToUrl) {
  ...
  const preview = versions
    ...
    .map((v) => navVersion(v, versionToUrl))
  return {
...
function navVersion(v, versionToUrl) {
  const navVersion = v.latest
    ? { latest: v.latest, url: v.url, displayVersion: v.displayVersion }
